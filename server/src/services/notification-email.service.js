...
export async function sendVettingApprovalRequestNotification({ demoRequest, requester }) { try { const { getVettingApprovalRequestHtml } = await import(./email-templates.service.js); const { sendEmail } = await import(./aws-ses.service.js); const superAdminEmail = process.env.SUPER_ADMIN_EMAIL?.trim() || support@classgrid.in; const html = getVettingApprovalRequestHtml({ requesterName: requester.name, institutionName: demoRequest.institutionName, adminName: demoRequest.adminName, adminEmail: demoRequest.adminEmail, city: demoRequest.city, dashboardUrl: https://superadmin.classgrid.in, leadId: demoRequest._id.toString() }); await sendEmail({ to: superAdminEmail, fromEmail: requester.email, fromName: ${requester.name} (Classgrid), subject: [Action Required] Sandbox Vetting Approval: , html, replyTo: requester.email }); } catch (err) { console.error([EmailNotification] sendVettingApprovalRequestNotification error:, err.message); } }
export async function sendVettingApprovedNotification({ demoRequest, assignee }) { try { const { getVettingApprovedHtml } = await import(./email-templates.service.js); const { sendEmail } = await import(./aws-ses.service.js); const superAdminEmail = process.env.SUPER_ADMIN_EMAIL?.trim() || support@classgrid.in; const html = getVettingApprovedHtml({ assigneeName: assignee.name, institutionName: demoRequest.institutionName, dashboardUrl: https://superadmin.classgrid.in, leadId: demoRequest._id.toString() }); await sendEmail({ to: assignee.email, fromEmail: superAdminEmail, fromName: Classgrid Vetting (Nikhil Shinde), subject: [Approved] Sandbox Vetting for , html, replyTo: superAdminEmail }); } catch (err) { console.error([EmailNotification] sendVettingApprovedNotification error:, err.message); } }
