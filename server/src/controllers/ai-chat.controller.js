            ...
            try {
                ...
                if (latestUser) {
                    ...
                    if (vercelConnected) {
                        ...
                        activeDescriptions.push(`- **Vercel & Website Deployment**: ✓ CONNECTED. ${vercelEmail}\n  **HOW TO DEPLOY WEBSITES (CRITICAL)**:\n  **IMPORTANT: YOU ONLY BUILD VANILLA HTML/CSS/JS SITES! DO NOT BUILD REACT OR NEXT.JS OR USE BUILD STEPS!**\n  **CRITICAL RULE FOR BOTH DEPLOYMENTS**: YOU MUST output the raw HTML/CSS/JS code directly in your chat response FIRST (using markdown \`\`\`html blocks) so the user can watch it being written in real-time! DO NOT write the code directly into a background tool! After showing the code in the chat, you can use the \`run_code\` tool to save it, then deploy it.\n  1. **Instant Classgrid Cloud (Cloudflare R2)**: After saving the files via \`run_code\`, use the \`cloudflare_r2_connector\` with operation \`upload_website\`. Generate a unique \`siteId\` (e.g. \`school-demo-123\`). Send the sandbox files in the \`files\` array. The site instantly goes live at https://<siteId>.sites.classgrid.in!\n  2. **GitHub + Vercel (Advanced)**: After saving the files via \`run_code\`, use \`github_workspace_connector\` to create repo/push code, then use \`vercel_connector\` (operation \`create_project\`) to link and deploy it. Set \`isClassgridManaged: true\` for both!`);
                    } else {
