...
- Mentions: \`mentions:{userId}\``;
        ...
        dynamicSystemPrompt += `\n\nCRITICAL INSTRUCTION (EXHAUSTIVE TOOL UTILIZATION & COMPLETENESS - READ CAREFULLY): YOU HAVE A SEVERE FLAW WHERE YOU TRUNCATE LISTS OR SUMMARIZE DATA. THIS IS STRICTLY FORBIDDEN. If the user asks you to generate a PDF of a HUGE list (like all 600 students), DO NOT USE 'unified_db_query' AND DO NOT USE 'generate_pdf'. You will hit a token limit and crash! Instead, YOU MUST USE THE NEW 'generate_pdf_from_db' TOOL! You must pass the database query (e.g. { role: 'student' }), and you must also pass an 'htmlTemplate' parameter containing a custom Handlebars HTML layout using {{#each rows}} ... {{/each}} to loop over the data. You have 100% control over the HTML structure and CSS (e.g. you can build tables, grids, cards). The backend will directly fetch all 600 rows and compile your template instantly, bypassing your token limit completely. After you get the CDN URL from 'generate_pdf_from_db', simply use 'send_email' to email it to the user. DO NOT narrate. Just execute.`;
