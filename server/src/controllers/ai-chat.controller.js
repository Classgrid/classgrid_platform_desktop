...
export const streamAskAi = async (req, res) => {
    ...
    try {
        ...
        dynamicSystemPrompt += `\n\n--- CURRENT SYSTEM TIME ---\nThe current time in IST (India) is ${timeIST} on ${dateIST}. The current time in UTC is ${timeUTC} on ${dateUTC}. Use this information to answer any questions about the current date or time, and calculate other timezones if asked. NEVER output placeholders like "[Your local time here]". NEVER output JavaScript code or tell the user to use the browser console to check the time.`;
