...
CRITICAL PRIVACY RULE: You must NEVER mention the name of this tool (internal_thought_process) in your conversational chat output. Never tell the user you are using it. Just use it silently.`;
            ...
            try {
                ...
                if (latestUser) {
                    ...
                    if (googleConnected) {
                        ...
                        activeDescriptions.push(`- **Google Workspace (Gmail, Calendar, Drive, Meet, Forms, Classroom)**: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ CONNECTED. ${googleEmail}Use 'google_workspace_connector' tool to list_emails, mark_email_read, list_events, create_event, list_drive_files, create_folder, create_form, get_form, read_drive_file, upload_drive_file, list_classroom_courses, list_classroom_assignments, list_classroom_submissions, list_classroom_teachers, list_classroom_announcements, list_classroom_topics, read_classroom_file. CRITICAL GMAIL RULE: If the user asks you to mark emails as read, you MUST ACTUALLY CALL the 'mark_email_read' tool for EACH email ID you are marking. NEVER refuse to mark emails as read, and NEVER hallucinate that you marked them. IMPORTANT WORKFLOW FOR DOCUMENTS: If the user asks you to read a file from Drive or Classroom, use \`read_drive_file\` or \`read_classroom_file\` to securely stage it in R2. The tool will return an R2 url. You MUST immediately call \`parse_document\` on that R2 url to read the text. To save a generated file to Drive, use \`upload_drive_file\` with the file URL.`);
                    }
