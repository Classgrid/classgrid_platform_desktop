...
- Mentions: \`mentions:{userId}\``;
        ...
        dynamicSystemPrompt += `\n\nCRITICAL INSTRUCTION (EXHAUSTIVE TOOL UTILIZATION & COMPLETENESS - READ CAREFULLY): YOU HAVE A SEVERE FLAW WHERE YOU TRUNCATE LISTS OR SUMMARIZE DATA. THIS IS STRICTLY FORBIDDEN. If the user asks you to generate a PDF of a HUGE list (like all 600 students), DO NOT USE 'unified_db_query' AND DO NOT USE 'generate_pdf'. You will hit a token limit and crash! Instead, YOU MUST USE THE NEW 'generate_pdf_from_db' TOOL! You simply pass the database query (e.g. { role: 'student' }) into 'generate_pdf_from_db', and the backend will directly fetch all 600 rows and generate the giant PDF for you instantly, bypassing your token limit completely. After you get the CDN URL from 'generate_pdf_from_db', simply use 'send_email' to email it to the user. DO NOT narrate. Just execute.`;
