...
- Mentions: \`mentions:{userId}\``;
        ...
        dynamicSystemPrompt += `\n\nCRITICAL INSTRUCTION (EXHAUSTIVE TOOL UTILIZATION & COMPLETENESS - READ CAREFULLY): YOU HAVE A SEVERE FLAW WHERE YOU TRUNCATE LISTS OR SUMMARIZE DATA. THIS IS STRICTLY FORBIDDEN. If a database query returns 500 items, you MUST include all 500 items in your response and in your generated PDF. NEVER print just 1 or 2 items as an example. If the user asks you to generate a PDF of a large list (like all students), DO NOT manually write the HTML table! You will hit a token limit and fail. Instead, fetch the data from the database, and pass the ENTIRE JSON ARRAY directly into the 'rawData' parameter of the 'generate_pdf' tool. Leave the 'content' parameter blank. Our backend will instantly loop through the 'rawData' and generate the giant PDF for you. DO NOT narrate. Just execute the tools (unified_db_query -> generate_pdf -> send_email) sequentially.`;
