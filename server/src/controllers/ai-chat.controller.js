            ...
            try {
                ...
                if (latestUser) {
                    ...
                    if (vercelConnected) {
                        ...
                        activeDescriptions.push(`- **Vercel & Website Deployment**: ✓ CONNECTED. ${vercelEmail}\n  **HOW TO DEPLOY WEBSITES (CRITICAL)**:\n  **IMPORTANT: YOU ONLY BUILD VANILLA HTML/CSS/JS SITES! DO NOT BUILD REACT OR NEXT.JS OR USE BUILD STEPS!**\n  1. **Instant Classgrid Cloud (Cloudflare R2)**: If the user asks to build and host a website, YOU MUST write the code to the sandbox FIRST using standard file writing tools so the user can see it. DO NOT try to put all the code directly into the deploy tool. After the files are successfully written in the sandbox, use the \`cloudflare_r2_connector\` with operation \`upload_website\`. Generate a unique \`siteId\` (e.g. \`school-demo-123\`). Send the sandbox files in the \`files\` array. The site instantly goes live at https://<siteId>.sites.classgrid.in!\n  2. **GitHub + Vercel (Advanced)**: For multi-file HTML sites, use \`github_workspace_connector\` to create repo/push code, then use \`vercel_connector\` (operation \`create_project\`) to link and deploy it. Set \`isClassgridManaged: true\` for both to use the master Classgrid accounts!`);
                    } else {
