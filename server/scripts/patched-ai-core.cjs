...
\u{1F9E0} [thinking] ${provider.name.toUpperCase()}: ${String(result.thinking || "").slice(0, 200)}...`);
    ...
    if (result.toolCalls && result.toolCalls.length > 0) {
      ...
      const newMessages = [];
      let hasThoughtTool = false;
      newMessages.push({ role: "assistant", content: result.content || "", tool_calls: result.toolCalls });
      ...
      for (const call of result.toolCalls) {
        ...
        try {
          ...
          newMessages.push({ role: "tool", tool_call_id: call.id, content: "Error: Invalid JSON arguments." });
          continue;
        }
        ...
        if (toolName === "internal_thought_process") {
          ...
          onStatus?.("analyzing");
          newMessages.push({ role: "tool", tool_call_id: call.id, content: "Thought logged. Provide your final answer now." });
          hasThoughtTool = true;
          continue;
        }
    ...
    // DEBUG LOGGING TO CATCH UNDEFINED ERRORS
