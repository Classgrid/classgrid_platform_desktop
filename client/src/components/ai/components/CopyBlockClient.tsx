...
export function CopyBlockClient({ text, label = 'copy' }: { text: string; label?: string }) {
  ...
  const getLabelInfo = () => {
    ...
    if (l === 'email') return { text: 'EMAIL', icon: Mail };
    if (l === 'prompt') return { text: 'PROMPT', icon: Terminal };
    if (l === 'message') return { text: 'MESSAGE', icon: MessageSquare };
    return { text: 'COPY', icon: Copy };
  };

  const { text: badgeText, icon: Icon } = getLabelInfo();
  ...
  return (
    <div className="group relative my-6 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#18181b] overflow-hidden text-sm shadow-sm dark:shadow-xl">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 dark:border-white/5 bg-white dark:bg-[#1f1f23]">
      ...
      {/* Content */}
      <div className="p-4 sm:p-5 text-slate-700 dark:text-zinc-300 text-[15px] leading-relaxed whitespace-pre-wrap font-sans">
        {text}
      </div>
    </div>
  );
