...
const SPRING_PRESS = { type: "spring", bounce: 0, duration: 0.2 };
...
function DitherMark({
  ...
  if (status === "complete") {
    return <Check aria-hidden="true" className="w-3.5 h-3.5" />;
  }

  if (status === "error") {
    return <CircleAlert aria-hidden="true" className="w-3.5 h-3.5" />;
  }
  ...
  return (
    <motion.span
      ...
      className="grid w-3.5 h-3.5 grid-cols-2 place-items-center gap-0.5"
    >
      <span className="w-1 h-1 rounded-[1px] bg-current" />
      <span className="w-1 h-1 rounded-[1px] bg-current opacity-55" />
      <span className="w-1 h-1 rounded-[1px] bg-current opacity-55" />
      <span className="w-1 h-1 rounded-[1px] bg-current" />
    </motion.span>
  );
...
function DitherField({
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 overflow-hidden bg-slate-100 dark:bg-white/5"
    >
      <canvas
        ref={canvasRef}
        className="absolute inset-0 w-full h-full text-slate-800 dark:text-white"
      />
    </motion.div>
  );
}
    ...
    >
      ...
      >
        <div
          ...
          style={{ aspectRatio }}
          className="relative isolate w-full overflow-hidden rounded-xl bg-slate-100 dark:bg-white/5"
        >
          <motion.div
            ...
            className={cn(
              "absolute inset-0 [&>*]:w-full [&>*]:h-full [&>*]:object-cover [&_img]:w-full [&_img]:h-full [&_img]:object-cover",
              mediaClassName,
            )}
          >
            {children}
          ...
          {resolution ? (
            <span className="absolute top-2 right-2 z-10 rounded-full bg-white/75 dark:bg-black/75 px-2 py-0.5 font-mono text-[10px] tabular-nums text-slate-500 dark:text-slate-400">
              {resolution}
            </span>
          ) : null}
        </div>
          ...
          <div className="mt-3 text-left">
            {showStatus ? (
              <div
                aria-live="polite"
                className={cn(
                  "flex min-h-5 items-center gap-2 text-sm font-medium text-slate-800 dark:text-slate-200",
        ...
        {status === "error" && onRetry ? (
          <motion.button
            ...
            className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-full px-3 text-sm font-medium text-slate-800 dark:text-slate-200 outline-none transition-colors hover:bg-slate-100 dark:hover:bg-white/10"
          >
