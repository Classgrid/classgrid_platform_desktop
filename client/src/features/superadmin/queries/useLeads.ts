...
export function useAssignLead() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, userId }: { id: string; userId?: string }) => leadsApi.assign(id, { userId }),
    onSuccess: () => qc.invalidateQueries({ queryKey: LEADS_KEY }),
  });
}
...
export function useUpdateMeetingNotes() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, notes, meetingStatus, isOrganizationVetted }: { id: string; notes?: string, meetingStatus?: string, isOrganizationVetted?: boolean }) =>
      leadsApi.updateMeetingNotes(id, { notes, meetingStatus, isOrganizationVetted }),
    onSuccess: () => qc.invalidateQueries({ queryKey: LEADS_KEY }),
  });
}
